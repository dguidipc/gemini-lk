...
#define FRAME_HEIGHT (480)  // pixel

#define PANEL_WIDTH  (480)  // pixel
#define PANEL_HEIGHT (800)  // pixel
...
static void init_lcm_registers(void)
{
	...
	if (0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "180", 3)) {
		data_array[0]= 0x00053902;
		data_array[1]= 0x01A0002a;
		data_array[2]= 0x0000003f;
		dsi_set_cmdq(data_array, 3, 1);

		data_array[0]= 0x00053902;
		data_array[1]= 0x0140012b;
...
static void lcm_update(unsigned int x, unsigned int y, unsigned int width, unsigned int height)
{
	unsigned int x0, x1;
	unsigned int y0, y1;
	unsigned char x0_MSB, x0_LSB, x1_MSB, x1_LSB;
	unsigned char y0_MSB, y0_LSB, y1_MSB, y1_LSB;
	unsigned int data_array[16];


	if (0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "180", 3)) {
		x0 = x + PANEL_WIDTH - width;
		y0 = y + PANEL_HEIGHT - height;
	} else {
		x0 = x;
		y0 = y;
	}
