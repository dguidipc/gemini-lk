...
#else
#error "UFS only supports GPT. Please turn on MTK_GPT_SCHEME_SUPPORT."
#endif
...
ssize_t partition_read(char *part_name, off_t offset, u8* data, size_t size)
{
	...
	read_addr = part_addr + (u64)offset;
	...
	len = dev->read(dev, read_addr, (uchar *) data, (int)size, partition_get_region(index));
...
ssize_t partition_write(char *part_name, off_t offset, u8* data, size_t size)
{
	...
	part_addr = partition_get_offset(index);
	part_size = partition_get_size(index);
	write_addr = part_addr + (u64)offset;
	...
	len = dev->write(dev, (uchar *) data, write_addr, (int)size, partition_get_region(index));
...
int partition_erase(char *part_name)
{
	...
	return dev->erase(0, erase_addr, size, part_id);
}
